...
public class GbCpu extends ClassicEmu
{
	...
	public final void reset( byte [] mem, int unmapped )
	{
		...
		//Evan - debug
		this.debug_mem = new String [this.mem.length];
		for (int i = 0; i < this.mem.length; i++) {
			this.debug_mem[i] = String.format("%02X", this.mem[i]);
	    }
		
		//was for copying bytes array 
//		System.arraycopy( this.mem, 0, this.debug_mem, 0, this.mem.length );
	}
	...
	loop:
		...
		{
			//Evan - debug
//			if (i++ > 50) {System.out.println(mem[ pages [pc >> pageShift] + pc ]);}
//			if (i++ > 0) {System.out.println(mem[ pages [sp >> pageShift] + sp ]);}
//			if (i++ > 0) {/*System.out.println( pages [de >> pageShift] + de );*/}
//			if (i >= 200) {System.exit(0);}
			
			//mark program counter
//			this.debug_mem[pages [pc >> pageShift] + pc] = "PC" + String.valueOf(i); //(byte) 500;//
//			System.out.println(this.i);
			if (i++ >= 7000) {
				System.exit(0);
				//output ram to file and quit
				try {
//					Files.write(Paths.get("C:/Users/Evan/Desktop/pokemon_gbs/hex_parse/cpu_output.txt"), this.debug_mem);
					
					String output = "";
					for (int j=0; j <  this.debug_mem.length; j++) {
						
					    if (j % 16 == 0) {
					    	output += String.format("%05X", j) + " ";
					    }
						output += this.debug_mem[j] + " ";

					    if (j % 16 == 15) {
					    	output += "\n";
					    }
					}
					PrintWriter out = new PrintWriter( "C:/Users/Evan/Desktop/pokemon_gbs/hex_parse/cpu_output.txt" );
				    out.println( output );
				    out.close();
				    
				    Files.write(Paths.get("C:/Users/Evan/Desktop/pokemon_gbs/hex_parse/ram_output.txt"), this.mem);
